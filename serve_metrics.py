...
def _login_html(
    ...
    *,
    ...
    html = (
        ...
        "max-height:min(85dvh,36rem);overflow:auto;"
        "border-radius:12px;padding:1.2rem 1.25rem;box-shadow:0 12px 40px rgba(0,0,0,.25);"
        "margin:auto}"
        ".modal h2{margin:0 0 .65rem;font-size:clamp(1.05rem,4.5vw,1.2rem);line-height:1.25}"
        ".modal p{margin:0 0 1rem;line-height:1.5;font-size:clamp(.92rem,3.8vw,1rem);"
        "text-wrap:pretty}"
        ".modal button{margin:0;width:100%}"
        "@media (max-width:420px){"
        ".shell{max-width:none}"
        ".modal{border-radius:10px;padding:1.05rem 1rem}"
        "}"
        "</style></head><body>"
        f"{modal_html}"
        '<div class="shell">'
